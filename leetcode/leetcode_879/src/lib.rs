...
pub fn profitable_schemes(n: i32, min_profit: i32, group: Vec<i32>, profit: Vec<i32>) -> i32 {
    ...
    for i in 1..=len {
        ...
        let earn = profit[i - 1] as usize;
        for j in (members..=n as usize).rev() {
            for k in (0..=min_profit as usize).rev() {
                dp[j][k] = (dp[j][k] + dp[j - members][k.saturating_sub(earn)]) % mod_num;
            }
        }
    }
    dp[n as usize][min_profit as usize]
}
