...
pub fn box_delivering(boxes: Vec<Vec<i32>>, ports_count: i32, max_boxes: i32, max_weight: i32) -> i32 {
    ...
    let mut p = vec![0; l + 1];
    let mut w = vec![0; l + 1];
    let mut neg = vec![0; l + 1];
    let mut ww = vec![0; l + 1];

    for i in 1..=l {
        p[i] = boxes[i-1][0];
        w[i] = boxes[i-1][1];
        if i > 1 {
            neg[i] = neg[i-1] + if p[i] == p[i-1] { 0 } else { 1 };
        }
        ww[i] = ww[i-1] + w[i];
    }

    let mut opt: VecDeque<usize> = VecDeque::new();
    opt.push_back(0);
    let mut f = vec![0; l + 1];
    let mut g = vec![0; l + 1];
    for i in 1..=l {
        while !opt.is_empty() && (i - opt.front().unwrap() > max_boxes as usize || ww[i] - ww[*opt.front().unwrap()] > max_weight) {
            opt.pop_front();
        }
