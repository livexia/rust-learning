...
impl ThroneInheritance {
    ...
    fn new(kingName: String) -> Self {
        let mut obj = ThroneInheritance {
            children: HashMap::new(),
            dead: HashSet::new(),
            king_name: kingName.clone(),
        };
        obj.children.entry(kingName.clone()).or_insert(vec![]);
        obj
    }
    ...
    fn get_inheritance_order(&self) -> Vec<String> {
        let mut res = Vec::new();
        self.dfs(&self.king_name, &mut res);
        res
    }

    fn dfs(&self, name: &str, res: &mut Vec<String>) {
        if !self.dead.contains(name) {
            res.push(name.to_string());
        }
        for name in self.children.get(name).unwrap_or(&vec![]) {
            self.dfs(name, res)
        }
    }
}
...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(
            order,
            vec_of_strings!["king", "andy", "matthew", "alex", "asha", "catherine"]
        );
    }
}
